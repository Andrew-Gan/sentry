...
class SimpleStateHasher(StateHasher):
    ...
    def merkle_tree(self, content, blockSize) -> hashing.Digest:
        buffer = checkCudaErrors(runtime.cudaMalloc(content.nbytes))
        buffer = np.array([int(buffer)], dtype=np.uint64)

        content = np.array([int(content.data_ptr())], dtype=np.uint64)
        blockSize = np.array([int(blockSize)], dtype=np.uint64)

        nThread = (content.nbytes + (blockSize-1)) // blockSize
        ...
        checkCudaErrors(driver.cuLaunchKernel(
            self.merkle_tree_pre, grid, 1, 1, block, 1, 1,
        ...
        while nThread > 0:
            ...
            checkCudaErrors(driver.cuLaunchKernel(
                self.merkle_tree_hash, grid, 1, 1, block, 1, 1,
                0, self.stream, args.ctypes.data, 0,
            ))

            checkCudaErrors(runtime.cudaMemcpy2D(buffer, 32, content,
                1024*blockSize, 1, grid, runtime.cudaMemcpyKind.cudaMemcpyDeviceToDevice))
            nThread //= 2048

        checkCudaErrors(runtime.cudaMemcpy(content, buffer, 32,
    ...
    def compute(self) -> hashing.Digest:
        ...
        self.merkle_tree(self._buffer, self._chunk_size)
